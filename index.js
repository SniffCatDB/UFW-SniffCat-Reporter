...
const reportIp = async ({ srcIp, dpt = 'N/A', proto = 'N/A', timestamp }, categories, comment) => {
	...
	try {
		...
		logger.success(`Reported ${srcIp} [${dpt}/${proto}]; Categories: ${categories}; Abuse: ${res.abuseConfidenceScore}%`);
		return true;
	} catch (err) {
		...
		if (status === 429 && JSON.stringify(err.response?.data || {}).includes('Daily rate limit')) {
			...
			const failureMsg = `Failed to report ${srcIp} [${dpt}/${proto}]; ${err.response?.data?.message ? err.response.data.message : err.message}`;
