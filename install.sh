...
😺 GitHub Issues : https://github.com/sefinek/UFW-SniffCat-Reporter/issues
...
if [ -d "UFW-SniffCat-Reporter" ] && [ ! -d "ufw-sniffcat" ]; then
    ...
    if command -v pm2 &>/dev/null; then
        if pm2 list | grep -q "ufw-sniffcat"; then
            echo "⏸️ Stopping ufw-sniffcat process..."
            pm2 stop ufw-sniffcat &>/dev/null || true
            pm2 delete ufw-sniffcat &>/dev/null || true
        fi
    fi
...
sudo mkdir -p /opt/ufw-sniffcat/tmp
...
existing_api_token=$(extract_config_value "SNIFFCAT_API_KEY")

while true; do
    read -e -i "$existing_api_token" -r -p "> 🔑 Please enter your SniffCat API token: " api_token
...
echo ""
echo "     Server ID identifies this machine in SniffCat reports and Discord notifications."
...
cache_file=$(ask_config "> 💾 Cache file path: " "${existing_cache_file:-./tmp/ufw-sniffcat-reporter.cache}" "")
...
echo "     Recommended: 12 hours. Minimum: 0.25 (15 minutes) due to SniffCat rate limits."
...
sed -i \
    ...
    -e "s|SNIFFCAT_API_KEY: .*|SNIFFCAT_API_KEY: '$api_token',|" \
...
echo -e "   After editing the configuration file, restart the process: pm2 restart ufw-sniffcat"
